...
const AddCollegeEdit = ({ showHide, hide, dataIndex, setEditIndex }) => {
  ...
  useEffect(() => {
    const data = user.college_university.find((item, i) => i === dataIndex);
    setInput({ ...data });
  }, [user, dataIndex]);
  ...
  const handleWorkSubmit = (e) => {
    ...
    if (!input.college_university) {
      ...
      const data = [...user.college_university];
    ...
    <>
      <form action="" onSubmit={handleWorkSubmit}>
        <input
          ...
          value={input.college_university}
        />
        <h4>Time Period</h4>
